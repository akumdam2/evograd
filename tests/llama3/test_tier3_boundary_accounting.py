"""Live-boundary invocation accounting for Llama-3: per site, in one namespace.

The defect these cover is the one Qwen3's equivalent covers, at Llama's shape: a
QKV-only patch installs the composite ``LlamaAttention`` adapter, which runs the
``attention`` boundary as well, so the tap sees more invocations than the
*patched* site alone declares. Comparing the two against one aggregate total
fails a run in which every checked invocation passed.

What is Llama's own here rather than inherited:

* the projection site is ``qkv_rope`` against ``llama3_qkv_rope``, so the
  carried pair is a different pair of names than Qwen3's;
* ``boundary.expected_counts`` is a *delegation* to ``sites.expected_counts``
  rather than a second table, and a report built from either must agree --
  Qwen3 still carries a hand-written dict, which is what the delegation exists
  to avoid repeating.

The derivation itself -- that patching either attention site makes both live,
and that the counts are ``L, L, L, 2L`` -- is already pinned in
``test_tier3.py::TestSiteRegistry`` and is deliberately not repeated. What is
here is the *accounting*: roles, per-site coverage, and every way it must fail.

Expectations are derived from the adapter grouping and the registry's own
counts. No test states a total.
"""
...
class TestTheTwoCountTablesAreOne(unittest.TestCase):
    ...
    def test_the_module_constant_is_the_published_depth(self):
        """``EXPECTED`` is the default a report falls back to when no plan is
...
class TestSitePlanRoles(unittest.TestCase):
    ...
    def test_the_plan_serializes_its_roles(self):
        plan = SitePlan.build((SITE_QKV,), layers=LAYERS).to_dict()
        self.assertEqual(plan["patched"], [SITE_QKV])
        self.assertEqual(plan["supporting"], [SITE_ATTENTION])
        self.assertEqual(plan["roles"][SITE_QKV], "patched")
        self.assertEqual(plan["roles"][SITE_ATTENTION], "supporting")
...
class TestGateReasonNamesTheSite(unittest.TestCase):
    ...
    def test_a_carried_site_failing_reads_differently_from_the_patched_one(self):
        ...
        reason = _boundary_reason(
            _full_run((SITE_QKV,), skip=((SITE_ATTENTION, 0),))
        )
        self.assertIn(SITE_ATTENTION, reason)
