...
class TestTimedBaselineAndGate(unittest.TestCase):
    ...
    def test_the_declared_tolerances_are_the_calibrated_ones(self):
        ...
        self.assertEqual(
            ...
            {
                # `out` picked one up when Llama-3-8B's 14336-wide intermediate
                # was measured; see the two tests below for what pays for it.
                "out": (2.4, 2.4),
                "dx": (2.3, 1.0),
    ...
    def test_the_out_multiplier_is_the_measured_shortfall_at_the_wide_shape(self):
        ...
        """
        op = get_op("qwen3_swiglu_mlp")
        from evograd.ops.level2.qwen3_swiglu_mlp import _REDUCTION_SCALED
    ...
    def test_what_the_out_multiplier_costs_the_correctness_grid(self):
        ...
        reference = forward_ref.qwen3_swiglu_mlp_forward_ref(*args).float()
