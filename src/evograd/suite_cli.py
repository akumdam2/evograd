...
# Canonical Python entry point: ``evograd.suite_cli``. The user-facing command is
# unchanged -- ``evograd suite`` -- and so are its arguments, defaults, help
# text, report schemas and exit codes.
#
# It lives at the root rather than in ``evograd.benchmark.operator_suite``
# because of what it does: it asks the benchmark which tasks and cases to run,
# and asks evaluation to run them. That is composition, and a package that owns
# definitions should not import a runner in order to perform it. With this
# module here, ``benchmark`` imports ``evaluation`` nowhere, and the
# dependency-direction test needs no exemption.
#
# The docstring above is the parser description, so it is the original text,
# character for character: ``evograd suite --help`` is part of the contract.
...
from evograd.evaluation.common.benchmark import (
...
def main(argv: list[str] | None = None) -> int:
    ...
    from evograd.evaluation.tier1.fair import (
        candidate_provider,
        declared_provider,
