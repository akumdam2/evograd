...
Whole-model workloads are benchmark definitions, not operators, and live under
:mod:`evograd.benchmark.topdown`.  This registry intentionally contains only
the forward/backward contracts that generation and evolution can optimize.

Operators are grouped into level directories matching their declarations. The
grouping is presentation only — the declaration remains authoritative and the
layout test prevents the two from drifting apart.
"""
...
def _collect_ops(
    ...
    *,
    ...
    for module_info in pkgutil.iter_modules(search_path, prefix):
        ...
        if op is None:
            # A grouping package rather than a task.
