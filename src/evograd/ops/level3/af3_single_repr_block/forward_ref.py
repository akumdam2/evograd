...
def _af3_single_repr_block(
    ...
    # ── attention ────────────────────────────────────────────────────────
    h = layer_norm(x, ln1_weight, ln1_bias, eps)
    ...
    # ── transition ───────────────────────────────────────────────────────
    h = layer_norm(x, ln2_weight, ln2_bias, eps)
...
def _layer_norm_fused(x, weight, bias, eps):
    """PyTorch's fused LayerNorm — one kernel instead of mean/sub/var/rsqrt."""
    return F.layer_norm(x, (x.shape[-1],), weight=weight, bias=bias, eps=eps)


def af3_single_repr_block_forward_ref(*args, **kwargs):
    """The definition: normalization spelled out in primitives. Signature and
    behaviour unchanged — AtenIR lowers this and the oracle differentiates it."""
    return _af3_single_repr_block(_layer_norm, *args, **kwargs)


def af3_single_repr_block_runtime_ref(*args, **kwargs):
    """The same block with PyTorch's fused LayerNorm, for timing the baseline."""
    return _af3_single_repr_block(_layer_norm_fused, *args, **kwargs)
