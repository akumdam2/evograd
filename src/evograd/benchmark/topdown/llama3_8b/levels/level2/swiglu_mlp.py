"""Extract and verify ``qwen3_swiglu_mlp`` from the verified Layer-16 replay.

    # describe the derived invocation (metadata only -- no tensors are written)
    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level2.swiglu_mlp derive \
    ...
    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level2.swiglu_mlp verify \
        --source results/llama3-level4/layer16.pt \
        --report results/llama3-level4/layer16-mlp-verify.json

    # measure what tolerance a correct BF16 implementation actually needs
    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level2.swiglu_mlp calibrate \
        --source results/llama3-level4/layer16.pt \
        --report results/llama3-level4/qwen3_swiglu_mlp-tolerance.json
