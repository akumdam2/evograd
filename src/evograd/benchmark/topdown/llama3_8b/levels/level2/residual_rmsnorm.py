"""Derive and verify ``fused_add_rms_norm`` from the canonical Layer-16 artifact.

    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level2.residual_rmsnorm derive \
        --source results/llama3-level4/layer16.pt \
        --metadata-out results/llama3-level4/layer16-residual.json

    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level2.residual_rmsnorm verify \
        --source results/llama3-level4/layer16.pt \
        --report results/llama3-level4/layer16-residual-verify.json

    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level2.residual_rmsnorm calibrate \
        --source results/llama3-level4/layer16.pt \
        --report results/llama3-level4/fused_add_rms_norm-tolerance.json
...
def declaration_problems(snapshot_path: Path | None = None) -> list[str]:
    ...
    from evograd.ops.level2.fused_add_rms_norm import LLAMA_FUSION_SITES
