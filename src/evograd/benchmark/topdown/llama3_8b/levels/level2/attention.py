"""Derive and verify ``qwen3_attention`` from the canonical Layer-16 artifact.

    # describe the derived invocation (metadata only -- no tensors are written)
    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level2.attention derive \
    ...
    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level2.attention verify \
        --source results/llama3-level4/layer16.pt \
        --report results/llama3-level4/layer16-attention-verify.json

    # measure what tolerance a correct implementation actually needs
    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level2.attention calibrate \
        --source results/llama3-level4/layer16.pt \
        --report results/llama3-level4/qwen3_attention-tolerance.json
...
def declaration_problems(snapshot_path: Path | None = None) -> list[str]:
    ...
    snapshot = load_snapshot(snapshot_path)
    harvest = snapshot["tasks"][TASK_NAME]
    declared, missing = _declared_case(get_op(TASK_NAME))
    if declared is None:
        return [missing]
