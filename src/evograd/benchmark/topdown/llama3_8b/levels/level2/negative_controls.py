...
TASKS = ("llama3_qkv_rope", "qwen3_attention", "qwen3_swiglu_mlp")
...
def dropped_term_fault(op, workload, clean, values, reference) -> dict[str, Any]:
    ...
    undetected. ``llama3_qkv_rope``'s ``dx`` is the case that forced this: with
    ...
    """
    from evograd.benchmark.topdown.llama3_8b.levels.level2.calibrate import production_results
...
def run_task(task: str, *, device: str = "cuda") -> dict[str, Any]:
    from evograd.benchmark.topdown.llama3_8b.levels.level2.calibrate import (
        production_results,
        reference_results,
    )
    ...
    op = get_op(task)
    workloads = list(op.benchmark_workloads(suite="llama_3_8b_observed"))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.benchmark.topdown.llama3_8b.levels.level2.negative_controls",
        description=__doc__,
    ...
    report = {
        "schema_version": "evograd-llama3-l2-negative-control/1",
        "device": args.device,
