"""Numerical calibration for the observed Llama-3 Level-2 boundaries.
...
* the declared **correctness** workloads -- small, CPU-runnable, what every test
  run gates on;
* the **llama_3_8b_observed** workloads -- the exact shapes the benchmark times,
  and the only ones at production width;
* the harvested **layer-16** invocation, whose tensors are the model's own
...
def _observed_workloads(op):
    try:
        return op.benchmark_workloads(suite="llama_3_8b_observed")
    except Exception:
        return ()
...
def _snapshot_hash() -> str | None:
    try:
        from evograd.benchmark.topdown.llama3_8b.harvest.snapshot import load
...
def _harvested_case(op, artifact: Path, *, device: str, repeats: int):
    ...
    builders = {
        ...
        "llama3_qkv_rope": _harvested_qkv,
    }
    ...
    try:
        ...
        return {
            "label": "harvested layer-16 (unavailable)",
            "error": f"{type(exc).__name__}: {exc}",
            "ok": True,
            "results": {},
            "worst_required_scale": 0.0,
        }
    record = case(op, "harvested layer-16 invocation", workload, values, repeats=repeats)
...
def _harvested_mlp(op, artifact: Path, *, device: str):
    from evograd.benchmark.topdown.llama3_8b.levels.level2.swiglu_mlp import derive_mlp_invocation
...
def _harvested_attention(op, artifact: Path, *, device: str):
    from evograd.benchmark.topdown.llama3_8b.levels.level2.attention import derive_attention_invocation
...
def _harvested_qkv(op, artifact: Path, *, device: str):
    from evograd.benchmark.topdown.llama3_8b.levels.level2.qkv_rope import derive_qkv_invocation
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.benchmark.topdown.llama3_8b.levels.level2.calibrate",
        description=__doc__,
    ...
    for name in ("inventory", "scaling"):
        ...
        sp.add_argument(
            "--artifact",
            type=Path,
            default=Path("results/llama3-level4/layer16.pt"),
...
def main(argv: list[str] | None = None) -> int:
    ...
    report: dict[str, Any] = {
        "schema_version": "evograd-llama3-l2-calibration/1",
