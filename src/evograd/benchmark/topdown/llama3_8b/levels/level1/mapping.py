"""The Llama-3-8B Level-1 mapping: check it, calibrate it, verify it.
...
Llama softmax workload would be a shape no step executes. And the observed
...
``layer16.pt`` stays the only tensor artifact. The canonical SDPA check re-derives
...
REPORT_SCHEMA = "evograd-llama3-level1-verify/1"
...
COMPOSES_INTO = {
    "linear_no_bias": ("llama3_qkv_rope", "qwen3_attention", "qwen3_swiglu_mlp"),
    # One entry, not Qwen3's two. Llama-3's only RMSNorm is the decoder's, which
    # the residual fusion owns; there is no per-head query/key norm inside the
    # projection boundary for a second edge to point at.
    "rmsnorm": ("fused_add_rms_norm",),
    "rope": ("llama3_qkv_rope",),
    "swiglu": ("qwen3_swiglu_mlp",),
    "causal_gqa_attention": ("qwen3_attention",),
    # The loss. Reached through `lm_head` rather than through a decoder layer,
    # so it composes into no Level-2 boundary and is checked on its own.
    "cross_entropy": (),
}
...
def summarize_mapping(report: dict[str, Any]) -> str:
    lines = [
        f"Llama-3-8B Level-1 mapping  (snapshot {report['snapshot_hash'][:16]}...)",
...
def run_calibration(op_name: str, *, device: str = "cuda", repeats: int = 3) -> dict[str, Any]:
    ...
    workloads = list(op.correctness) + list(op.benchmark_workloads("llama_3_8b_observed"))
    cases = []
        ...
        cases.append(
            {
                ...
                and workload.provenance.model == "llama_3_8b",
                "results": results,
    ...
    return {
        "schema_version": "evograd-llama3-level1-tolerance/1",
        "task": op_name,
        "device": device,
        "metric": (
...
# --------------------------------------------------------------------------
# canonical SDPA verification, derived from layer16.pt
# --------------------------------------------------------------------------
...
def run_loss_check(*, device: str = "cuda", snapshot_path: Path | None = None) -> dict[str, Any]:
    ...
    freshly initialised model's loss sits near ``ln(vocab)``, which for Llama-3's
    ...
    report = {
        "schema_version": "evograd-llama3-level1-loss/1",
        "task": "cross_entropy",
        "dims": case.dims,
        "dtype": case.dtype,
        ...
        "canonical_level4_loss": 12.594182968139648,
...
def run_cross_entropy_check(*, device: str = "cuda", spec=None) -> dict[str, Any]:
    ...
    return {
        "schema_version": "evograd-llama3-level1-cross-entropy/1",
        "status": "pass" if not failures else "fail",
        "failures": failures,
        "task": "cross_entropy",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.benchmark.topdown.llama3_8b.levels.level1.mapping",
        description=__doc__,
