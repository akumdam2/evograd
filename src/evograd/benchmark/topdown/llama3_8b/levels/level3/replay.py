"""Replay one captured decoder layer, alone, and check it against the full model.
    ...
    PYTHONPATH=src python -m evograd.benchmark.topdown.llama3_8b.levels.level3.replay \
        --artifact results/llama3-level4/layer16.pt \
        --report results/llama3-level4/layer16-replay.json

This process constructs exactly one ``LlamaDecoderLayer``. It never builds
``LlamaModel`` or ``LlamaForCausalLM``, and the report proves it by scanning the
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.benchmark.topdown.llama3_8b.levels.level3.replay",
        description=__doc__,
