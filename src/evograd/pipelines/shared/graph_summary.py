"""AtenIR graph summarization for pipeline prompts and contexts.

Two variants ported from the old repo:

* :func:`summarize_backward_graph` / ``..._file`` — Pipeline A's prompt
  summary (from ``pipeline/fusion_agent/graph_summary.py``), keyed on
  ``predecessor_ids``/``scalar_args``.
* :func:`summarize_graph` + :func:`summarize_node` — the richer
  ``args_ordered``-based summary Pipeline B's lowering context uses (from
  ``pipeline/primitive_atenir_lowering_agent/graph_summary.py``).
"""
...
def summarize_backward_graph(graph: dict[str, Any], max_nodes: int | None = None) -> str:
...
def summarize_backward_graph_file(path: Path, max_nodes: int | None = None) -> str:
    return summarize_backward_graph(load_graph(path), max_nodes=max_nodes)
...
def summarize_graph(graph: dict[str, Any], max_nodes: int | None = None) -> str:
    ...
    for index, node in enumerate(selected_calls):
        ...
        scalar_values = [
            e["value"] for e in ao if e.get("kind") == "scalar" and e.get("value") is not None
        ]
        lines.append(
            f"{index}. `{node['name']}` target=`{node.get('target')}` "
