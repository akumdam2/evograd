"""Auto-generated by Pipeline B (handwritten dispatch) — a self-contained,
dispatch-free Triton program for one AtenIR graph.

Only the Triton kernels actually used by this graph are included below (pulled
transitively from evograd/atenir/primitive_triton/), wrapped in an EVOLVE-BLOCK so
OpenEvolve can rewrite/fuse them. There is no string-matching op dispatch table
and no runtime registry: every call in `run_graph_program` below the block is a
direct call to one of these kernels, with any baked scalar arguments inlined as
literal defaults.

`run_graph_program` is the literal, unrolled call sequence for this graph — one
statement per AtenIR call_function node, in topological order.
"""

from __future__ import annotations

import torch
import triton
import triton.language as tl
import triton.language.extra.cuda.libdevice as libdevice

# EVOLVE-BLOCK-START

HAS_TRITON = True
_BLOCK = 1024

def mul_tt(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    a, b = _bc(a, b)
    out = torch.empty_like(a)
    _mul_kernel[_grid(a.numel())](a, b, out, N=a.numel(), BLOCK=_BLOCK)
    return out

def _bc(*tensors):
    """Broadcast all tensors to a common shape and make contiguous."""
    broadcast = torch.broadcast_tensors(*tensors)
    return tuple((t.contiguous() for t in broadcast))

@triton.jit
def _mul_kernel(a_ptr, b_ptr, out_ptr, N, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    a = tl.load(a_ptr + offs, mask=mask, other=0.0)
    b = tl.load(b_ptr + offs, mask=mask, other=0.0)
    tl.store(out_ptr + offs, a * b, mask=mask)

def _grid(n: int):
    import math
    return (math.ceil(n / _BLOCK),)

def _row_sum(a: torch.Tensor, keepdim: bool) -> torch.Tensor:
    return _row_reduce(a, _row_sum_kernel, a.dtype, keepdim)

def _row_reduce(a: torch.Tensor, kernel, out_dtype, keepdim: bool) -> torch.Tensor:
    assert a.ndim == 2
    a = a.contiguous()
    R, C = a.shape
    out = torch.empty(R, device=a.device, dtype=out_dtype)
    BLOCK_C = min(_next_pow2(C), 65536)
    kernel[R,](a, out, R, C, BLOCK_C=BLOCK_C)
    return out.unsqueeze(1) if keepdim else out

def _next_pow2(n: int) -> int:
    p = 1
    while p < n:
        p <<= 1
    return p

@triton.jit
def _row_sum_kernel(a_ptr, out_ptr, R, C, BLOCK_C: tl.constexpr):
    """sum([R, C], dim=1) → [R]  (one program per row)."""
    row = tl.program_id(0)
    cols = tl.arange(0, BLOCK_C)
    mask = cols < C
    a = tl.load(a_ptr + row * C + cols, mask=mask, other=0.0)
    tl.store(out_ptr + row, tl.sum(a.to(tl.float32), axis=0).to(a.dtype))

def div_scalar(a: torch.Tensor, scalar) -> torch.Tensor:
    a = a.contiguous()
    out = torch.empty_like(a)
    _div_scalar_kernel[_grid(a.numel())](a, out, float(scalar), N=a.numel(), BLOCK=_BLOCK)
    return out

@triton.jit
def _div_scalar_kernel(a_ptr, out_ptr, scalar, N, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    a = tl.load(a_ptr + offs, mask=mask, other=0.0)
    tl.store(out_ptr + offs, a / scalar, mask=mask)

def _col_sum(a: torch.Tensor, keepdim: bool) -> torch.Tensor:
    return _col_reduce(a, _col_sum_kernel, a.dtype, keepdim)

def _col_reduce(a: torch.Tensor, kernel, out_dtype, keepdim: bool) -> torch.Tensor:
    assert a.ndim == 2
    a = a.contiguous()
    R, C = a.shape
    out = torch.empty(C, device=a.device, dtype=out_dtype)
    BLOCK_R = min(_next_pow2(R), 65536)
    kernel[C,](a, out, R, C, BLOCK_R=BLOCK_R)
    return out.unsqueeze(0) if keepdim else out

@triton.jit
def _col_sum_kernel(a_ptr, out_ptr, R, C, BLOCK_R: tl.constexpr):
    """sum([R, C], dim=0) → [C]  (one program per column)."""
    col = tl.program_id(0)
    rows = tl.arange(0, BLOCK_R)
    mask = rows < R
    a = tl.load(a_ptr + rows * C + col, mask=mask, other=0.0)
    tl.store(out_ptr + col, tl.sum(a.to(tl.float32), axis=0).to(a.dtype))

def add_scalar(a: torch.Tensor, scalar) -> torch.Tensor:
    a = a.contiguous()
    out = torch.empty_like(a)
    _add_scalar_kernel[_grid(a.numel())](a, out, float(scalar), N=a.numel(), BLOCK=_BLOCK)
    return out

@triton.jit
def _add_scalar_kernel(a_ptr, out_ptr, scalar, N, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    a = tl.load(a_ptr + offs, mask=mask, other=0.0)
    tl.store(out_ptr + offs, a + scalar, mask=mask)

def rsqrt(a: torch.Tensor) -> torch.Tensor:
    a = a.contiguous()
    out = torch.empty_like(a)
    _rsqrt_kernel[_grid(a.numel())](a, out, N=a.numel(), BLOCK=_BLOCK)
    return out

@triton.jit
def _rsqrt_kernel(a_ptr, out_ptr, N, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    a = tl.load(a_ptr + offs, mask=mask, other=1.0).to(tl.float32)
    tl.store(out_ptr + offs, 1.0 / tl.sqrt(a), mask=mask)

def mul_scalar(a: torch.Tensor, scalar) -> torch.Tensor:
    a = a.contiguous()
    out = torch.empty_like(a)
    _mul_scalar_kernel[_grid(a.numel())](a, out, float(scalar), N=a.numel(), BLOCK=_BLOCK)
    return out

@triton.jit
def _mul_scalar_kernel(a_ptr, out_ptr, scalar, N, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    a = tl.load(a_ptr + offs, mask=mask, other=0.0)
    tl.store(out_ptr + offs, a * scalar, mask=mask)

def pow_scalar(a: torch.Tensor, exp) -> torch.Tensor:
    a = a.contiguous()
    exp_f = float(exp)
    if exp_f == 0.0:
        return torch.ones_like(a)
    if exp_f == 1.0:
        return a.clone()
    if exp_f == 2.0:
        return mul_tt(a, a)
    if exp_f == 3.0:
        return mul_tt(mul_tt(a, a), a)
    out = torch.empty_like(a, dtype=torch.float32)
    _pow_scalar_kernel[_grid(a.numel())](a, out, exp_f, N=a.numel(), BLOCK=_BLOCK)
    return out.to(a.dtype)

@triton.jit
def _pow_scalar_kernel(a_ptr, out_ptr, exp, N, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    a = tl.load(a_ptr + offs, mask=mask, other=1.0).to(tl.float32)
    tl.store(out_ptr + offs, tl.exp(exp * tl.log(a)), mask=mask)

def add_tt(a: torch.Tensor, b: torch.Tensor, alpha: float=1.0) -> torch.Tensor:
    a, b = _bc(a, b)
    out = torch.empty_like(a)
    if alpha != 1.0:
        b = mul_scalar(b, alpha)
    _add_kernel[_grid(a.numel())](a, b, out, N=a.numel(), BLOCK=_BLOCK)
    return out

@triton.jit
def _add_kernel(a_ptr, b_ptr, out_ptr, N, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    a = tl.load(a_ptr + offs, mask=mask, other=0.0)
    b = tl.load(b_ptr + offs, mask=mask, other=0.0)
    tl.store(out_ptr + offs, a + b, mask=mask)

def _k0(a: torch.Tensor, _dims=[-1], _keepdim=True) -> torch.Tensor:
    if HAS_TRITON and a.ndim == 2:
        R, C = a.shape
        dims_norm = [d % a.ndim for d in _dims]
        if dims_norm == [1]:
            total = _row_sum(a, _keepdim)
            return div_scalar(total, float(C))
        if dims_norm == [0]:
            total = _col_sum(a, _keepdim)
            return div_scalar(total, float(R))
    return a.mean(dim=_dims if _dims else None, keepdim=_keepdim)

def _k1(a, s=1e-05):
    return add_scalar(a, s)

def _k2(a: torch.Tensor, _dims=[0], _keepdim=True) -> torch.Tensor:
    if HAS_TRITON and a.ndim == 2:
        dims_norm = [d % a.ndim for d in _dims]
        if dims_norm == [1]:
            return _row_sum(a, _keepdim)
        if dims_norm == [0]:
            return _col_sum(a, _keepdim)
    return a.sum(dim=_dims if _dims else None, keepdim=_keepdim)

def _k3(a: torch.Tensor, _dims=[1], _keepdim=True) -> torch.Tensor:
    if HAS_TRITON and a.ndim == 2:
        dims_norm = [d % a.ndim for d in _dims]
        if dims_norm == [1]:
            return _row_sum(a, _keepdim)
        if dims_norm == [0]:
            return _col_sum(a, _keepdim)
    return a.sum(dim=_dims if _dims else None, keepdim=_keepdim)

def _k4(a, s=-0.5):
    return mul_scalar(a, s)

def _k5(a, exp_val=3.0):
    return pow_scalar(a, exp_val)

def _k6(a, s):
    return div_scalar(a, float(s))


@triton.jit
def _strided_copy_kernel(
    src_ptr,
    out_ptr,
    N,
    s0, s1, s2, s3, s4, s5, s6, s7,  # output sizes, rank padded to 8
    t0, t1, t2, t3, t4, t5, t6, t7,  # source strides (elements), 0 = broadcast
    BLOCK: tl.constexpr,
):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < N
    rem = offs
    src_off = tl.zeros_like(offs)
    idx = rem % s7
    rem = rem // s7
    src_off += idx * t7
    idx = rem % s6
    rem = rem // s6
    src_off += idx * t6
    idx = rem % s5
    rem = rem // s5
    src_off += idx * t5
    idx = rem % s4
    rem = rem // s4
    src_off += idx * t4
    idx = rem % s3
    rem = rem // s3
    src_off += idx * t3
    idx = rem % s2
    rem = rem // s2
    src_off += idx * t2
    idx = rem % s1
    rem = rem // s1
    src_off += idx * t1
    idx = rem % s0
    src_off += idx * t0
    val = tl.load(src_ptr + src_off, mask=mask, other=0.0)
    tl.store(out_ptr + offs, val, mask=mask)


def strided_copy(a, out_shape, src_strides):
    """Materialize a contiguous tensor of ``out_shape`` gathered from ``a``.

    ``src_strides[i]`` is the element stride of ``a`` along output dim ``i``
    (0 for broadcast dims). Rank up to 8.
    """
    out_shape = [int(s) for s in out_shape]
    src_strides = [int(s) for s in src_strides]
    out = torch.empty(out_shape, device=a.device, dtype=a.dtype)
    n = out.numel()
    if n == 0:
        return out
    pad = 8 - len(out_shape)
    sizes = [1] * pad + out_shape
    strides = [0] * pad + src_strides
    grid = ((n + 1023) // 1024,)
    _strided_copy_kernel[grid](a, out, n, *sizes, *strides, BLOCK=1024)
    return out


def permute_copy(a, dims):
    """Contiguous permute: equivalent to a.permute(dims).contiguous()."""
    dims = [int(d) % a.ndim for d in dims]
    return strided_copy(a, [a.shape[d] for d in dims], [a.stride(d) for d in dims])


def expand_copy(a, shape):
    """Contiguous expand: equivalent to a.expand(shape).contiguous().

    Follows aten.expand semantics: ``shape`` may prepend new leading dims, and
    -1 keeps the source size. Size-1 source dims broadcast with stride 0.
    """
    shape = [int(s) for s in shape]
    lead = len(shape) - a.ndim
    out_shape, strides = [], []
    for i, s in enumerate(shape):
        if i < lead:
            out_shape.append(s)
            strides.append(0)
            continue
        src_dim = i - lead
        src_size = a.shape[src_dim]
        size = src_size if s == -1 else s
        out_shape.append(size)
        strides.append(0 if (src_size == 1 and size != 1) else a.stride(src_dim))
    return strided_copy(a, out_shape, strides)


def clone_copy(a):
    """Contiguous clone via the strided-copy kernel."""
    return strided_copy(a, list(a.shape), list(a.stride()))



_FORWARD_SPEC = 'evograd.ops.rmsnorm.forward_ref:rmsnorm_forward_ref'


def _load_forward_callable():
    from evograd.opdecl.importing import resolve_callable

    return resolve_callable(_FORWARD_SPEC)


def _forward_with_saved_impl(x, weight, eps=1e-5):
    # Conservative seed: only save original forward inputs. OpenEvolve may
    # replace this with saved intermediates.
    y = _load_forward_callable()(x, weight, eps)
    return y, (x.contiguous(), weight.contiguous())


def _backward_from_saved_impl(dy, saved_tensors, eps=1e-5):
    _ = eps
    x, weight = saved_tensors[:2]
    return run_graph_program(
        dy.contiguous(),
        x.contiguous(),
        weight.contiguous(),
    )


def rmsnorm_forward_with_saved(x, weight, eps=1e-5):
    return _forward_with_saved_impl(x, weight, eps)


def rmsnorm_backward_from_saved(dy, saved_tensors, eps=1e-5):
    return _backward_from_saved_impl(dy, saved_tensors, eps)
# EVOLVE-BLOCK-END


def run_graph_program(t_arg0_1, t_arg1_1, t_arg2_1):
    t_alias = t_arg1_1  # alias: aten.alias.default (alias)
    t_alias_1 = t_arg2_1  # alias_1: aten.alias.default (alias)
    t_sym_size_int = int(t_arg2_1.shape[0])  # sym_size_int: aten.sym_size.int
    t_sym_size_int_1 = int(t_arg1_1.shape[1])  # sym_size_int_1: aten.sym_size.int
    t_mul = mul_tt(t_alias, t_alias)  # mul: aten.mul.Tensor
    t_mean = _k0(t_mul)  # mean: aten.mean.dim
    t_add = _k1(t_mean)  # add: aten.add.Tensor
    t_rsqrt = rsqrt(t_add)  # rsqrt: aten.rsqrt.default
    t_alias_2 = t_rsqrt  # alias_2: aten.alias.default (alias)
    t_mul_1 = mul_tt(t_alias, t_rsqrt)  # mul_1: aten.mul.Tensor
    t_mul_2 = mul_tt(t_mul_1, t_alias_1)  # mul_2: aten.mul.Tensor
    t_sym_size_int_2 = int(t_arg1_1.shape[0])  # sym_size_int_2: aten.sym_size.int
    t_mul_3 = mul_tt(t_arg0_1, t_mul_1)  # mul_3: aten.mul.Tensor
    t_mul_4 = mul_tt(t_arg0_1, t_alias_1)  # mul_4: aten.mul.Tensor
    t_sum_1 = _k2(t_mul_3)  # sum_1: aten.sum.dim_IntList
    t_view = t_sum_1.reshape(t_sym_size_int,)  # view: aten.view.default
    t_mul_5 = mul_tt(t_mul_4, t_alias)  # mul_5: aten.mul.Tensor
    t_mul_6 = mul_tt(t_mul_4, t_rsqrt)  # mul_6: aten.mul.Tensor
    t_sum_2 = _k3(t_mul_5)  # sum_2: aten.sum.dim_IntList
    t_alias_3 = t_alias_2  # alias_3: aten.alias.default (alias)
    t_mul_7 = _k4(t_sum_2)  # mul_7: aten.mul.Scalar
    t_pow_1 = _k5(t_alias_3)  # pow_1: aten.pow.Tensor_Scalar
    t_mul_8 = mul_tt(t_mul_7, t_pow_1)  # mul_8: aten.mul.Tensor
    t_expand = expand_copy(t_mul_8, (t_sym_size_int_2, t_sym_size_int_1))  # expand: aten.expand.default
    t_div = _k6(t_expand, t_sym_size_int_1)  # div: aten.div.Scalar
    t_mul_9 = mul_tt(t_div, t_alias)  # mul_9: aten.mul.Tensor
    t_mul_10 = mul_tt(t_div, t_alias)  # mul_10: aten.mul.Tensor
    t_add_1 = add_tt(t_mul_6, t_mul_10)  # add_1: aten.add.Tensor
    t_add_2 = add_tt(t_add_1, t_mul_9)  # add_2: aten.add.Tensor
    return (t_add_2, t_view)
