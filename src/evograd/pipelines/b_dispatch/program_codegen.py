...
kernel via `evograd.atenir.primitive_triton.dispatch.make_kernel` at generation time —
...
class _ProgramBuilder:
    ...
    def __init__(self) -> None:
        self.submodules: dict[str, Any] = {
            name: importlib.import_module(f"evograd.atenir.primitive_triton.{name}")
            for name in _SUBMODULE_NAMES
        }
        self.dispatch_mod = importlib.import_module("evograd.atenir.primitive_triton.dispatch")
...
def generate_dispatch_program(graph: dict) -> str:
    ...
    if layout_used:
        from evograd.pipelines.b_dispatch.layout_kernels import embeddable_source

        kernel_library = kernel_library.rstrip("\n") + "\n\n\n" + embeddable_source()

    header = '''"""Auto-generated by Pipeline B (handwritten dispatch) — a self-contained,
...
transitively from evograd/atenir/primitive_triton/), wrapped in an EVOLVE-BLOCK so
...
def generate_autograd_pair_program(graph: dict, *, forward: str, op) -> str:
    ...
    ``op`` is the :class:`evograd.opdecl.activity.OpDecl` for the target
    operator; the wrapper is rendered from its activity annotations.
    """
