"""Pipeline C: forward-source-only autograd-pair synthesis ablation.

Baseline for measuring how much AtenIR context helps: the LLM sees only the
forward reference source. Tracks per-call token usage into cost_summary.json.
"""
...
def _verify(config: ForwardOnlyConfig, program_path: Path) -> dict:
...
def _write_cost_summary(
    output_dir: Path, calls: list[dict[str, Any]], attempts: int, passed: bool
) -> None:
    usage_totals: dict[str, int] = {}
    for call in calls:
        _merge_usage(usage_totals, call)
    summary = {
        "agent": "c_forward_only",
...
def synthesize_forward_only(config: ForwardOnlyConfig) -> int:
    ...
    print("Forward-only: plan synthesis")
    ...
    for attempt in range(1, config.max_attempts + 1):
        ...
        print(f"Forward-only: codegen attempt {attempt}/{config.max_attempts}")
        response = _call_llm(
            config=config,
            prompt=prompt,
        ...
        if report_passed(report):
            ...
            print(f"Forward-only synthesis passed. Best program: {best_path}")
            return 0
    ...
    print(f"Forward-only synthesis failed after {config.max_attempts} attempts")
    return 1
