"""Run a canonical training step once and describe what happened.
...
def workload_info(spec: WorkloadSpec) -> dict[str, Any]:
    # `type(spec)()` is this workload's canonical run: a spec subclass's field
    # defaults are its reference execution, so a shrunk variant still reports
    # the id it deviates from without anything here naming a model.
    return {
        "workload_id": spec.workload_id,
        "workload_hash": spec.workload_hash,
        "canonical": spec.is_canonical,
        "canonical_workload_id": type(spec)().workload_id,
...
def run_smoke(workload: Level4Workload, spec: WorkloadSpec | None = None) -> SmokeReport:
    ...
    report = SmokeReport(
        ...
        schema_version=workload.smoke_schema,
    )
    ...
    try:
        ...
        if spec.device.startswith("cuda") and not torch.cuda.is_available():
            raise RuntimeError(
                "this workload runs on CUDA and no CUDA device is visible; "
