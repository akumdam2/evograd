"""Run the canonical Llama-3-8B training smoke and write its JSON report.
    ...
    PYTHONPATH=src python -m evograd.bench.workloads.llama3 \
...
run non-canonical: the report says so, the id changes, and a banner is printed
to stderr. Nothing here can turn the cache or gradient checkpointing on -- those
are refused by the spec, not by this parser.
"""
...
def add_override_arguments(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    """The debug overrides, quoting Llama-3's own canonical values."""
