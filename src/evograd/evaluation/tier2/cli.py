"""Tier-2 operator/autograd evaluation: one nn.Module per provider and shape.

    python -m evograd.evaluation.tier2.cli --op layernorm \
...
def _specs(args, candidate_module):
    from evograd.evaluation.tier2.runner import default_provider_specs, identity_control_specs
...
def _run_one_case(args, op, workload, only: str | None = None):
    from evograd.evaluation.tier2.runner import DEFAULT_REP_MS, DEFAULT_WARMUP_MS, run_case
...
def _run_isolated(argv: list[str], index: int, provider: str | None = None) -> dict:
    ...
    try:
        process = subprocess.run(
            [sys.executable, "-m", "evograd.evaluation.tier2.cli", *argv,
