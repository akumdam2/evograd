"""Measure the canonical Llama-3 model's own numerical noise, then bound it.
    ...
    PYTHONPATH=src python -m evograd.evaluation.tier3.workloads.llama3_8b.calibrate run \
...
def _repatch(workload, model, kind: str):
    ...
    """
    from evograd.evaluation.tier3.workloads.llama3_8b.sites import PatchedModel, expected_counts, patch_model
...
def _counters():
    from evograd.evaluation.tier3.workloads.llama3_8b.sites import SiteCounters
...
def _run_isolated(argv: list[str]) -> dict[str, Any]:
    ...
    try:
        process = subprocess.run(
            [sys.executable, "-m", "evograd.evaluation.tier3.workloads.llama3_8b.calibrate",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.evaluation.tier3.workloads.llama3_8b.calibrate",
        description=__doc__,
