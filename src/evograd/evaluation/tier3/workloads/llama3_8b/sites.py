"""Drop-in replacement sites inside a real ``LlamaForCausalLM``.
...
training loop noticing:
    ...
    qkv_rope          projections, then RoPE
...
untouched. The parameters are not copied, they are the same objects, so
...
also makes patching reversible by simply rebuilding the workload.
...
**Two differences from the Qwen3 registry, and neither is cosmetic.**

*No per-head query/key normalization.* ``LlamaAttention`` projects, reshapes,
transposes and rotates; there is no ``q_norm``/``k_norm``. The site is therefore
``qkv_rope`` against the ``llama3_qkv_rope`` declaration, with four active
arguments rather than six, and the two site names deliberately differ so that
neither registry's identity control, baseline discovery or report can reach the
other's sites.

*No sliding window.* ``LlamaAttention.forward`` does not pass ``sliding_window``
to the attention interface and the module has no such attribute, so
:func:`production_attention` must not invent one. Passing Qwen3's spelling here
would raise on a model that has never had the attribute -- which is the good
case -- or silently select a different attention kernel, which is not.
"""
...
from evograd.evaluation.tier3.patch import ModulePatch, PatchProvenance, Site, SiteRegistry

#: The four site names. They are the ``--sites`` spellings and the report keys.
#: ``qkv_rope``, not Qwen3's ``qkv_norm_rope``: a different operator with a
#: different argument list, and a name collision would let a Qwen3 kernel set be
#: restricted onto a Llama model without complaint.
SITE_QKV = "qkv_rope"
...
#: ``qkv_rope`` and ``attention`` are one LlamaAttention adapter with two
...
#: Site -> the operator declaration that site is gated against.
SITE_OPS: dict[str, str] = {
    SITE_QKV: "llama3_qkv_rope",
    # Structurally the same boundary Qwen3 declares -- causal GQA SDPA followed
    # by o_proj -- at Llama's dims. The mathematics is shared (one
    # implementation, in benchmark.topdown.common.level2_references); the task
    # is not. Each architecture owns its own key, so a report row, a candidate
    # program and a calibration file are never ambiguous about whose widths
    # produced them.
...
#: The benchmark suite carrying the shapes a canonical Llama step runs.
#: **Empty until a harvest has been run**, which is why :func:`_observed`
#: tolerates its absence rather than failing the registry build.
...
#: Distinct from Qwen3's prefix, so a model that somehow carried both would show
#: two states rather than one overwriting the other.
_TAG = "_evograd_llama3_"
...
class SiteCounters:
    ...
    observation. A patch that matches 32 modules but runs 31 of them is a
    different defect from one that matches 31, and only counting both finds it.
    """
...
class ResidualCarrier:
    ...
    ``llama3_residual_rmsnorm`` is the only site here that is not one module. Its two
...
def production_qkv_rope(module, hidden_states, cos, sin):
    """Exactly ``LlamaAttention.forward``'s first six lines, on ``module``.
    ...
    No ``q_norm``/``k_norm``. That absence is the whole difference between this
    boundary and Qwen3's, and it is why a shared adapter would have to branch on
    an architecture rather than on a site.
    """
...
def production_attention(module, query, key, value, attention_mask, input_shape, **kwargs):
    """Exactly ``LlamaAttention.forward``'s attention interface and output projection.

    Note what is *not* forwarded: ``sliding_window``. ``LlamaAttention`` has no
    such attribute and does not pass one, so reproducing Qwen3's call here would
    be a different call, not the same one written twice.
    """
...
def production_residual_rmsnorm(norm_call, branch, residual):
    ...
    ``LlamaDecoderLayer.forward`` removes the question. ``norm_call`` is the
    original ``LlamaRMSNorm`` computation -- for ``model.norm``, whose forward
...
def _observed(op_name: str):
    ...
    Returns ``()`` when the Llama harvest has not been run, which is the state
    this workload is in today. An empty preflight suite means the site is gated
    on the declaration's own correctness grid only -- weaker than Qwen3's gate,
    and the difference is exactly one snapshot.
    """
    from evograd.benchmark import get_task

    try:
        return tuple(get_task(op_name).benchmark_workloads(suite=OBSERVED_SUITE))
    except Exception:  # a declaration without the suite, or no snapshot yet
        return ()


def build_registry() -> SiteRegistry:
    """Llama-3's four sites, with any calibrated shapes attached to preflight."""
    return SiteRegistry(
        name="llama_3_8b",
        sites=(
            Site(SITE_QKV, SITE_OPS[SITE_QKV], production_qkv_rope,
                 preflight=_observed(SITE_OPS[SITE_QKV])),
            Site(SITE_ATTENTION, SITE_OPS[SITE_ATTENTION], production_attention,
                 preflight=_observed(SITE_OPS[SITE_ATTENTION])),
            Site(SITE_MLP, SITE_OPS[SITE_MLP], production_swiglu_mlp,
                 preflight=_observed(SITE_OPS[SITE_MLP])),
            Site(SITE_RESIDUAL, SITE_OPS[SITE_RESIDUAL], production_residual_rmsnorm,
                 preflight=_observed(SITE_OPS[SITE_RESIDUAL])),
        ),
    )
...
def _is_production(kernel, site: str) -> bool:
    return kernel is llama3_sites().require(site).default
...
def structural_identity_kernels(registry: SiteRegistry | None = None):
    ...
    registry = registry or llama3_sites()
...
def bound_pair_identity_kernels(ops, sites: tuple[str, ...] | None = None,
    ...
    The deployment path an evolved kernel will take. Gated by the declared
    tolerances, not by bitwise equality, because the declared reference and the
    production spelling are different computations.
    """
    from evograd.evaluation.tier3.patch import identity_control_kernels

    return identity_control_kernels(ops, sites, registry=registry or llama3_sites())
...
def _probe_outputs(state, outputs):
    ...
    there reads the boundary gradient plus the operator's own internal one.
    ...
    and only the *downstream* consumers see it. The aliases exist only while a
    validator is attached; nothing measured ever sees one.
    """
...
def _probe_inputs(state, mapping: dict, names: tuple[str, ...]):
    ...
    ``_probe_outputs`` reasoning, the other way round. Layer 0's
    ``hidden_states`` feeds both ``input_layernorm`` and the post-attention
    fusion's residual, so a hook there is the sum of two operators' emissions
    and belongs to neither. The operator consumes the alias instead, and the
    alias has exactly one consumer -- this call.

    Only activations are aliased; a parameter is read off its module by the
    production spelling and cannot be substituted.
    """
    listener = state.get("tap")
    if listener is None or not getattr(listener, "probes", False):
        return mapping, None
...
def _tap(state, site: str, inputs: dict, outputs, key=_UNSET, emitted=None):
    ...
    production branch and on a candidate's alike.
    """
    listener = state.get("tap")
    if listener is None:
        return outputs
...
def _attention_forward(self, hidden_states, position_embeddings=None,
    ...
    return is its ``(attn_output, attn_weights)`` pair, so ``LlamaDecoderLayer``
    calls this exactly as it called the original.
    """
    state = getattr(self, _TAG + "state")
    counters = state["counters"]
    if past_key_values is not None:
        raise NotImplementedError(
            "the Llama-3 tier-3 attention adapter is a training-step boundary; "
    ...
    if qkv_kernel is None or _is_production(qkv_kernel, SITE_QKV):
        query, key, value = _tap(
            ...
            production_qkv_rope(self, qkv_inputs["x"], cos, sin),
            emitted=qkv_emitted,
        )
    else:
    ...
    if attention_mask is not None:
        raise NotImplementedError(
            "the attention declaration covers the mask-free causal branch "
    ...
    if not state.get("positions_checked"):
        if not bool(torch.equal(table[0], table[1])):
            raise NotImplementedError(
                ...
                "[1, T, D] cos/sin cannot represent them; llama3_qkv_rope is "
                "declared for the unpadded canonical workload"
            )
...
def install_attention_adapter(module, kernels: dict[str, Callable],
    ...
    """Rebind ``forward`` on a live ``LlamaAttention``. Nothing is copied."""
...
def _fused_site(state, kernel, production, category, branch, residual,
    ...
    """
    inputs = {  # `r` is what llama3_residual_rmsnorm calls the residual.
...
def _decoder_layer_forward(self, hidden_states, attention_mask=None, position_ids=None,
    ...
    *next* layer, so this returns the un-added MLP branch and leaves the pair in
    ...
    stays unfused, which is why there are 64 sites and not 65.
    """
    state = getattr(self, _TAG + "state")
    ...
    if past_key_values is not None or use_cache:
        raise NotImplementedError(
            "the Llama-3 tier-3 decoder adapter is a training-step boundary; "
            "cache-enabled execution is not supported"
        )
...
def _final_norm_forward(self, hidden_states):
    ...
    un-added branch, and the residual it belongs to is in the carrier.
    """
    state = getattr(self, _TAG + "state")
    kernel = state["kernels"].get(SITE_RESIDUAL)
    production = kernel is None or _is_production(kernel, SITE_RESIDUAL)
...
def patch_model(model, kernels, counters: SiteCounters | None = None,
    ...
    """Install every requested site on a built ``LlamaForCausalLM``.
    ...
    if attention_sites:
        ...
        _require(len(touched), layers, attention_sites, "LlamaAttention")
    ...
    if SITE_MLP in selected:
        ...
        _require(len(touched), layers, [SITE_MLP], "LlamaMLP")
...
def module_patches() -> tuple[ModulePatch, ...]:
    ...
    return (
        ModulePatch(SITE_QKV, _named("LlamaAttention"), lambda o, k: o,
                    sites=(SITE_ATTENTION,)),
        ModulePatch(SITE_MLP, _named("LlamaMLP"), lambda o, k: o),
        ModulePatch(SITE_RESIDUAL, _named("LlamaDecoderLayer"), lambda o, k: o),
    )
