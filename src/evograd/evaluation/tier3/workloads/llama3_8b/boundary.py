...
SCHEMA_VERSION = "evograd-llama3-t3-boundary/2"
...
#:
#: Evaluated from the published layer count rather than written out, so a
#: literal cannot drift from the architecture the way a hand-copied 28 did.
#: ``expected_counts`` is the authority; this is its canonical value, kept
#: because ``BoundaryReport.to_dict`` needs a default when no plan is supplied.
...
class SitePlan:
    ...
    The distinction is load-bearing. Patching ``qkv_rope`` installs the
    composite LlamaAttention adapter, which then runs the ``attention`` boundary
...
def validate_all_invocations(workload, kernels, *, data_seed: int = 0) -> dict[str, Any]:
    ...
    def observed_cases(op):
        """This workload's harvested configurations, or ``()`` if unharvested."""
        from .sites import OBSERVED_SUITE

        try:
            return op.benchmark_workloads(suite=OBSERVED_SUITE)
        except KeyError:
            return ()
    ...
    # Which tolerance population every comparison above was judged against.
    # "declared_correctness_grid" means the harvested shapes were unavailable,
    # so this run proved less than a harvested one would have.
    summary["tolerance_source"] = (
        OBSERVED_SUITE
        if all(observed_cases(op_lookup(site)) for site in report.counts)
