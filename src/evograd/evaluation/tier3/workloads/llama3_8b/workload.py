"""The canonical Llama-3-8B training step, as a tier-3 workload.
    ...
    evograd tier3-bench --model llama_3_8b --structural-identity
...
serializable fields; :meth:`Llama3Workload.to_config` writes them out and
...
``build_model``/``make_inputs`` are the same functions level 4 uses.

One thing is new relative to the spec: a **data seed** separate from the
workload's own. The tier needs a fresh batch per loss-trajectory step, and
drawing those from the workload seed would change the canonical identity. So the
...
linearly in the layer count and the loss kernel's memory does not depend on it
at all, so a reduced-layer run is the way to iterate. It is not canonical, and
``spec.is_canonical`` says so.
"""
...
from evograd.benchmark.topdown.llama3_8b.levels.level4.model import build_model, effective_settings, make_inputs
from evograd.benchmark.topdown.llama3_8b.levels.level4.spec import CANONICAL, WorkloadSpec
from .sites import (
    PatchedModel,
    SiteCounters,
    expected_counts,
...
class Llama3Workload:
    ...
    projection site is ``qkv_rope`` against ``llama3_qkv_rope``, Qwen3's is
    ``qkv_norm_rope`` against a different declaration -- so neither one's
    identity control, baseline discovery or report can reach the other's sites.
    """
    ...
    def __post_init__(self) -> None:
        ...
        self.site_registry = llama3_sites()
    ...
    @classmethod
    def from_config(cls, config: dict[str, Any]) -> "Llama3Workload":
    ...
    def site_preflight(self, kernels, *, device: str = "cuda") -> dict[str, Any]:
        ...
        return {"ok": True, "sites": list(kernels.patched),
                "checked": "declared correctness grid plus any observed shapes"}
...
def from_config(config: dict[str, Any]) -> Llama3Workload:
