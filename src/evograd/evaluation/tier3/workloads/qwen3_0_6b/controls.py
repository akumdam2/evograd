"""Negative controls: does the calibrated gate still reject a wrong model?
    ...
    PYTHONPATH=src python -m evograd.evaluation.tier3.workloads.qwen3_0_6b.controls \
...
def _isolated(argv: list[str]) -> dict[str, Any]:
    ...
    try:
        process = subprocess.run(
            [sys.executable, "-m", "evograd.evaluation.tier3.workloads.qwen3_0_6b.controls",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.evaluation.tier3.workloads.qwen3_0_6b.controls",
        description=__doc__,
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "gate_stages": list(__import__(
            "evograd.evaluation.tier3.workloads.qwen3_0_6b.gate", fromlist=["STAGES"]).STAGES),
        "rejected_at": {
