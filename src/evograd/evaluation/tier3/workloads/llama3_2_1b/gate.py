"""The whole-model correctness gate tier 3 runs before it times a Llama provider.

Site preflight proves a kernel correct at its declared shapes. It cannot prove
that 160 of them assembled into a 32-layer model still train, and the failure it
misses is the expensive one: a provider that is right on a small grid, wrong in
composition, and fast.

So a non-eager provider must clear all of these before a timer starts:

1. its site-level tier-1 preflight, including any workload-supplied observed
   shapes -- that gate already exists and is not repeated here;
2. purity: the provider is a function of its arguments;
3. every live boundary invocation matches its declaration;
4. every loss, logit, gradient, stepped parameter and checked optimizer state is
   finite, and one **untimed** canonical forward/backward/AdamW step fits inside
   the calibrated per-role envelope, measured against *both* references: the
   original eager model, and the bound-pair path an evolved kernel replaces;
5. its fresh-batch loss trajectory, over exactly the horizon the policy was
   calibrated for, fits the trajectory bound;
6. its invocation counts and ``PatchProvenance`` match what was requested.
...
**No Llama calibration exists yet**, so :func:`load_policy` refuses today. That
refusal is the correct behaviour rather than a gap to route around: the
alternative is an ungated timing, which is not cheaper than no timing but worse.
"""
...
#: Where the calibration lives. Tracked as a *result*, not as code, and per
#: model: Qwen3's file at ``results/qwen3-level4/`` describes Qwen3's noise
#: floor and cannot stand in for this one.
DEFAULT_ARTIFACT = Path("results/llama3-level4/t3-numerics-calibration.json")
...
def load_policy(path: Path | None = None, *, require_environment: bool = True):
    ...
    if not path.is_file():
        raise CalibrationUnavailable(
            f"no numerics calibration at {path}. Run `python -m "
            "evograd.evaluation.tier3.workloads.llama3_2_1b.calibrate run` on "
            "this machine; tier 3 will not time a Llama provider on an "
            "uncalibrated gate."
        )
...
def capture_step(model, optimizer, ids, labels) -> dict[str, Any]:
    ...
    **Host memory is the binding constraint at Llama-3.2-1B's size.** Three
    ...
    for name, param in model.named_parameters():
        # Held on the host: the comparisons are streaming reductions that do
        # not need these resident on the device.
        ...
        # update perturbs nothing.
        stored[name] = param.detach().to("cpu", copy=True)
        step = state.get("step")
        steps[name] = float(step) if step is not None else None
    return {
        # On the host for the same reason the updates are: a
        # [2, 2048, 128256] logits tensor in float32 is 2 GiB, and comparing
        # two of them is peak the device does not need to carry.
...
#: and the kind that gives it its own envelope namespace.
_FAMILIES = (
...
def _compare(candidate: dict[str, Any], reference: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    """
    from evograd.evaluation.tier3.gate.numerics import compare_tensor
    ...
    for family, prefix, kind in _FAMILIES:
        for name, expected in reference[family].items():
            ...
            if got is None:
                ...
                samples.append({
                    "name": f"{prefix}{name}", "role": role, "kind": kind,
                    "group": numerics.group_key(kind, role),
...
def build_references(workload, *, policy: NumericsPolicy | None = None, data_seed: int = 0):
    ...
    the negative controls do exactly that -- pays for them once.
    """
...
def _boundary_reason(report: dict[str, Any]) -> str:
    ...
    if report.get("duplicate_ids"):
        return f"an invocation was recorded twice: {report['duplicate_ids'][0]}"
    missing = report.get("missing_or_extra") or {}
    if missing:
        # Per site, and say which role each is, so "the QKV kernel never ran"
        # never reads the same as "the carried attention boundary did not".
    ...
    if failures:
        first = failures[0]
        return (f"{report['failure_count']} of {report['checked_invocations']} "
                f"invocations disagreed with the declaration; worst at "
                f"{first['id']} on {first['result']} "
