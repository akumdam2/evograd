"""Measure the canonical Llama-3 model's own numerical noise, then bound it.

    PYTHONPATH=src python -m evograd.evaluation.tier3.workloads.llama3_2_1b.calibrate run \
        --report results/llama3-level4/t3-numerics-calibration.json
...
CHILD_TIMEOUT = int(os.environ.get("EVOGRAD_LLAMA3_CAL_TIMEOUT", "3600"))
...
def _repatch(workload, model, kind: str):
    ...
    """
    from evograd.evaluation.tier3.workloads.llama3_2_1b.sites import PatchedModel, expected_counts, patch_model
...
def _counters():
    from evograd.evaluation.tier3.workloads.llama3_2_1b.sites import SiteCounters
...
def run_cell(seed: int, repeat: int, *, workload_config: dict[str, Any],
    ...
    workload = Llama3Workload.from_config(workload_config)
...
def _run_isolated(argv: list[str]) -> dict[str, Any]:
    ...
    handle, path = tempfile.mkstemp(prefix="evograd_llama3_cal_", suffix=".json")
    os.close(handle)
    try:
        process = subprocess.run(
            [sys.executable, "-m", "evograd.evaluation.tier3.workloads.llama3_2_1b.calibrate",
...
def build_policy(cells: list[dict[str, Any]], *, workload: Llama3Workload,
    ...
    return NumericsPolicy(
        ...
        notes={
            ...
            "residual_rmsnorm_spelling": (
                "llama3_residual_rmsnorm's declared runtime_forward normalizes with "
                "F.rms_norm while LlamaRMSNorm casts to bfloat16 before the "
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.evaluation.tier3.workloads.llama3_2_1b.calibrate",
        description=__doc__,
