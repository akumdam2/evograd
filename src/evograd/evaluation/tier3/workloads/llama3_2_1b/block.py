"""Llama-3.2-1B's decoder layer as a tier-3 block.

The block boundary is the original ``LlamaDecoderLayer`` call: one
...
Four sites live inside it, each invoked once:
    ...
    qkv_rope          the projections and RoPE (no per-head norm: Llama's own)
...
``layer8.pt`` artifact -- the model's own weights, arguments and upstream
...
This is the Llama counterpart of ``workloads/qwen3_0_6b/block.py``: the same
executor contract, this model's sites, builders, artifact reader and rotary
embedding. The shared executor knows neither.
"""
...
ARCHITECTURE = "llama_3_2_1b"
BLOCK_KIND = "decoder_layer"
OBSERVED_SUITE = "llama_3_2_1b_observed"
BOUNDARY = ("LlamaDecoderLayer: hidden_states in -> hidden_states out; input_layernorm "
...
def _revision() -> str:
    from evograd.benchmark.topdown.llama3_2_1b.levels.level4.model import TESTED_TRANSFORMERS
    ...
    try:
        ...
        live = "unavailable"
    return (f"meta-llama/Llama-3.2-1B config.json as pinned by levels.level4.spec.LLAMA_3_2_1B; "
...
def _block_layer_forward(self, hidden_states, attention_mask=None, position_ids=None,
                         ...
                         **kwargs):
    """``LlamaDecoderLayer.forward`` with the post-attention add+norm as a site.
    ...
    if past_key_values is not None or use_cache:
        raise NotImplementedError(
            "the Llama-3 block adapter is a training boundary; cache-enabled execution "
            "is not supported"
        )
...
@dataclass
class Llama3Block:
    """One Llama-3 decoder layer, captured or config-derived, as the executor sees it."""
    ...
    registry: Any = field(default_factory=llama3_sites)
    observed_suite: str | None = OBSERVED_SUITE

    # -- build / prepare ----------------------------------------------------

    def build(self, *, device: str) -> BuiltBlock:
        from evograd.benchmark.topdown.llama3_2_1b.levels.level3.prepare import build_single_layer
    ...
    def prepare(self, built: BuiltBlock, *, device: str) -> BlockInvocation:
        from evograd.benchmark.topdown.llama3_2_1b.levels.level3.prepare import verify_layout
...
def captured_case(artifact_path: str | Path, *, layer_index: int | None = None,
                  canonical: bool = True) -> Llama3Block:
    ...
    case = BlockCase(
        ...
        boundary=BOUNDARY, sites=dict(llama3_sites().site_ops), excluded=EXCLUDED,
    ...
    return Llama3Block(case=case, arch=arch, layer_index=int(identity["layer_index"]),
...
def config_case(*, batch: int, seq: int, layer_index: int, dtype: str, seed: int = 0,
                arch_overrides: Mapping[str, Any] | None = None) -> Llama3Block:
    ...
    the rotary tables come from the model's own ``LlamaRotaryEmbedding`` on
    ``position_ids = arange(T)`` -- including Llama-3.2's rope scaling, since
    it is built from the same config -- which is what the full model would hand
    the layer. Weights are the layer's default initialisation under the same
    seed and are kept on the adapter so every build loads the same numbers.
    """
    from evograd.benchmark.topdown.llama3_2_1b.levels.level3.prepare import (
        build_config, build_single_layer)
    from evograd.benchmark.topdown.llama3_2_1b.levels.level4.model import DTYPES
    from evograd.benchmark.topdown.llama3_2_1b.levels.level4.spec import LLAMA_3_2_1B

    # A deep copy: Transformers standardises ``rope_scaling`` *in place* when it
    # builds the config, and the pinned declaration must not carry that.
    arch = copy.deepcopy({**LLAMA_3_2_1B, **dict(arch_overrides or {})})
    ...
    recipe = {
        ...
        "weights": {"kind": "seeded_default_init", "seed": seed, "note":
                    "LlamaDecoderLayer default initialisation under torch.manual_seed(seed) on CPU"},
        "inputs": {"hidden_states": "randn(B, T, H) from torch.Generator(seed)",
                   "position_ids": "arange(T)[None]",
                   "position_embeddings": "LlamaRotaryEmbedding(config)(hidden_states, position_ids)",
    ...
    case = BlockCase(
        ...
        sites=dict(llama3_sites().site_ops), excluded=EXCLUDED, state_contract=dict(STATE_CONTRACT),
        weights_hash=tensor_tree_hash(weights, label="weights"),
    ...
    return Llama3Block(case=case, arch=arch, layer_index=layer_index, dtype=dtype,
                       weights=weights, invocation_cpu=invocation, capture=None)


def from_args(args) -> Llama3Block:
    ...
    if source == "config":
        from evograd.benchmark.topdown.llama3_2_1b.harvest.snapshot import REPRESENTATIVE_LAYER
        ...
        return config_case(
            ...
            layer_index=layer_index if layer_index is not None else REPRESENTATIVE_LAYER,
