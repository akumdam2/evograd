"""Proving the Llama-3 tier-3 sites are drop-in, before anything is timed.

    PYTHONPATH=src python -m evograd.evaluation.tier3.workloads.llama3_2_1b.validate \
        structural --report results/llama3-level4/t3-structural.json
...
def _run_model(workload: Llama3Workload, kernels, *, batch, learning_rate: float):
...
def compare_full_model(workload: Llama3Workload, kernels, *, label: str,
...
def validate_boundaries(workload: Llama3Workload, *, layer: int | None = None) -> dict[str, Any]:
    ...
    def listener(site, key, inputs, outputs):
        ...
        # is the right demand. `llama3_residual_rmsnorm`'s is not: it normalizes with
        # `F.rms_norm`, where LlamaRMSNorm casts to bfloat16 before multiplying
        ...
        workload_case = _tolerance_case(op)
        record = {
            "site": site,
...
def _workload(args) -> Llama3Workload:
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.evaluation.tier3.workloads.llama3_2_1b.validate",
        description=__doc__,
    ...
    report: dict[str, Any] = {
        "schema_version": "evograd-llama3-tier3-identity/1",
...
def _model_tolerance(workload: Llama3Workload, kernels) -> tuple[float, float]:
    ...
    for site in kernels.patched:
        ...
        for name in op.output_names:
            a, r = op.tolerance_for(_tolerance_case(op), name)
            atol, rtol = max(atol, a), max(rtol, r)
    return atol, rtol


def _tolerance_case(op):
    """The declared workload whose tolerances this model's shapes fall under.

    The harvested configuration when one exists, and the widest declared
    benchmark case otherwise. No Llama harvest has been run, so today this is
    always the latter -- a real gate, but a narrower one than Qwen3's, and the
    difference is exactly one ``harvest/snapshot.json``.
    """
