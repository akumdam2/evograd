...
**Why this is the workload's and not shared.** The mechanics -- the backward-scaling
autograd function, the optimizer-state faults, the sensitivity table -- live in
:mod:`evograd.evaluation.tier3.gate.faults`. Constructing a *kernel* fault does not
...
* ``layer_subset`` and ``single_layer`` are wrong in half the layers and in one
  of them. The fraction is the control: 1/32nd is where a pooled whole-model
...
from evograd.evaluation.tier3.gate.faults import (  # noqa: F401  (re-export)
...
def _build(fault: "Fault", registry) -> Callable:
    ...
    if fault.name == "single_layer":
        ...
        def kernel(*args):
            # One layer out of 32. Averaged over the whole model this is a
...
def catalogue(magnitudes=(0.001, 0.005, 0.02)) -> list[Fault]:
    ...
    faults += [
        ...
        Fault("single_layer", SITE_MLP, 0.02,
              "2% high in one layer out of 32, exact everywhere else"),
    ]
    return faults


# ── this workload's own optimizer-side faults ────────────────────────────────
#
...
# This workload therefore requires a two-ULP perturbation of the *stored*
# parameter,
...
def observability(clean: dict[str, torch.Tensor],
    ...
    """
    from evograd.evaluation.tier3.gate.numerics import role_of
