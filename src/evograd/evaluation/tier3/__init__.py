...
be read and changed on its own:
    ...
    model.py          **what** is measured -- the ``TrainingWorkload`` protocol,
                      and ``ModuleWorkload`` for bringing your own ``nn.Module``
    patch.py          **how a kernel gets in** -- ``KernelSet``, the patch sites,
                      ``bind`` wrapping, module surgery, and the identity control
    runner.py         **how it is measured** -- verify, build, step, time, memory,
                      loss agreement, report

A workload -- the model, and which of its places are patchable -- lives in
``evaluation/tier3/workloads/<name>/``, never here. This half knows how to measure a
...
from evograd.evaluation.tier3.model import (  # noqa: F401  (re-export)
    ModuleWorkload,
    TrainingWorkload,
    build_with_provenance,
    site_registry_for,
)
from evograd.evaluation.tier3.patch import (  # noqa: F401  (re-export)
    NO_SITES,
    KernelSet,
    KernelSource,
    ModulePatch,
