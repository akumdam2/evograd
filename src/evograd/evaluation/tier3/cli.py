"""Tier-3 evaluation: evolved kernels inside a real training step.

    python -m evograd.evaluation.tier3.cli --model <workload> \
...
#: here, so adding one is an entry in the tier-3 workload registry plus a package, not an
...
def _baseline_kernels(baseline: str, ops, registry):
    ...
    """
    from evograd.evaluation.tier3 import KernelSet, KernelSource, kernel_from_pair, patch
...
def tier3_adapter(name):
    ...
    from evograd.evaluation.tier3.workloads import UnknownWorkload
    from evograd.evaluation.tier3.workloads import tier3_adapter as lookup
...
def _run_isolated(argv: list[str], provider: str, timeout: int) -> dict:
    ...
    try:
        process = subprocess.run(
            [sys.executable, "-m", "evograd.evaluation.tier3.cli", *argv,
