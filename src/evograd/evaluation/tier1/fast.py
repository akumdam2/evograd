"""Tier 1 (direct pair), `fast` measurement mode — evolution fitness.

The cheap half of tier 1. ``fair.py`` is the same interface measured under the
``fair`` mode, and it is the direct-pair path used for published numbers; this
...
report produced here records the mode in its existing ``protocol`` field.
