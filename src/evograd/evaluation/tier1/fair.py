"""Tier 1 (direct pair), `fair` measurement mode.

Two axes run through benchmark and evaluation, and it helps to keep them apart:

    *tier*      what is measured — a kernel pair, an operator through the
                autograd engine (``tier2``), a training step (tier 3)
    *mode*      how carefully — ``fast`` for the evolutionary search,
                ``fair`` for anything anyone reads

This file is tier 1 x fair. ``fast.py`` is tier 1 x fast: the same interface
...
# `from evograd.evaluation.tier1.fair import PairProvider` is the spelling in existing
# callers, CLIs and tests.
from evograd.evaluation.common.provider import (  # noqa: F401  (re-export)
    PairProvider,
    TensorSnapshot,
...
from evograd.evaluation.common.provider import saved_state_report as _saved_state_report  # noqa: F401
