"""Verifying the primitives Qwen3-0.6B runs against what the model computed.

Which primitives those are, at which shapes, belongs to
:mod:`evograd.benchmark.topdown.qwen3_0_6b.levels.level1.manifest`. What is here is the judgment: the SDPA
boundary captured from a live layer and checked against the declared
contract, the cross-entropy head checked against the model's own loss, a
whole-step loss sanity check, and the reports that carry each verdict.
"""
...
from evograd.benchmark.topdown.qwen3_0_6b.levels.level1.manifest import COMPOSES_INTO, Level1Error, mapping
from evograd.benchmark.topdown.qwen3_0_6b.levels.level2.attention.capture import preserve_layout_cpu
from evograd.benchmark.topdown.qwen3_0_6b.levels.level3.artifact import ArtifactError, load_canonical
from evograd.benchmark.topdown.qwen3_0_6b.levels.level3.prepare import prepare_layer
...
def run_cross_entropy_check(*, device: str = "cuda", spec=None) -> dict[str, Any]:
    ...
    from evograd.benchmark.topdown.qwen3_0_6b.levels.level4.model import build_model, make_inputs, require_transformers, training_step
    from evograd.evaluation.workloads.qwen3_0_6b.level3.replay import compare_tensors
