"""Does the calibrated gate still reject a wrong kernel?
    ...
    PYTHONPATH=src python -m evograd.benchmark.topdown.qwen3_0_6b.levels.level2.negative_controls \
...
def dropped_term_fault(op, workload, name, values, reference) -> dict[str, Any]:
    ...
    """
    from evograd.benchmark.topdown.qwen3_0_6b.levels.level2.calibrate import production_results
...
def run_task(task: str, *, device: str = "cuda") -> dict[str, Any]:
    from evograd.benchmark.topdown.qwen3_0_6b.levels.level2.calibrate import (
        production_results,
        reference_results,
    )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.benchmark.topdown.qwen3_0_6b.levels.level2.negative_controls",
        description=__doc__,
