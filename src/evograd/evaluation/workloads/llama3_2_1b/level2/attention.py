"""Judging an implementation of ``llama3_attention`` against the captured boundary.

The case itself belongs to :mod:`evograd.benchmark.topdown.llama3_2_1b.levels.level2.attention.capture`.
...
from evograd.benchmark.topdown.llama3_2_1b.levels.level2.attention.capture import (
...
REPORT_SCHEMA = "evograd-llama3-attention-verify/1"
...
def run_verify(
    ...
    *,
    ...
    from evograd.benchmark.topdown.llama3_2_1b.levels.level2.attention.reference import (
        llama3_attention_forward_production,
        llama3_attention_forward_ref,
    )
    ...
    production = _pair_pass(llama3_attention_forward_production, **tensors)
    if device.startswith("cuda"):
        torch.cuda.synchronize()
    ...
    if include_dense_reference:
        dense = _pair_pass(llama3_attention_forward_ref, **tensors)
        dense_comparisons = {
    ...
    if noise_repeats == 0:
        ...
        noise["measured"] = True
        passes = [
            _pair_pass(llama3_attention_forward_production, **tensors) for _ in range(noise_repeats)
        ]
        noise["results"] = {
            name: _max_noise(
...
def _calibration_case(label, dtype, tensors, multipliers, repeats: int = 3):
    ...
    reference = _pair_pass(llama3_attention_forward_ref, **tensors)
    production = _pair_pass(llama3_attention_forward_production, **tensors)
    results = {
    ...
    for _ in range(max(repeats - 1, 0)):
        again = _pair_pass(llama3_attention_forward_production, **tensors)
        for name in RESULT_NAMES:
            noise[name] = max(
                noise[name],
                required_tolerance(
...
def run_calibration(
    ...
    *,
    ...
    return {
        "schema_version": "evograd-llama3-attention-tolerance/1",
        "task": TASK_NAME,
        "device": device,
        "metric": (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.benchmark.topdown.llama3_2_1b.levels.level2.attention",
        description=__doc__,
