"""Judging an implementation of ``llama3_qkv_rope`` against the captured boundary.

The case itself belongs to :mod:`evograd.benchmark.topdown.llama3_2_1b.levels.level2.qkv_rope.capture`.
...
from evograd.benchmark.topdown.llama3_2_1b.levels.level3.artifact import ArtifactError, load_canonical
from evograd.evaluation.workloads.llama3_2_1b.level3.replay import declared_gate, required_tolerance
from evograd.benchmark.topdown.llama3_2_1b.harvest.snapshot import load as load_snapshot
from evograd.benchmark.topdown.llama3_2_1b.levels.level2.qkv_rope.capture import (
    GRAD_NAMES,
    OUTPUT_NAMES,
    QkvExtractionError,
    TASK_NAME,
    ...
    derive_qkv_invocation,
)

REPORT_SCHEMA = "evograd-llama3-qkv-verify/1"
...
def run_verify(
    ...
    *,
    ...
    from evograd.benchmark.topdown.llama3_2_1b.levels.level2.qkv_rope.reference import (
        llama3_qkv_rope_forward_production,
        llama3_qkv_rope_forward_ref,
    )

    from evograd.evaluation.workloads.llama3_2_1b.level3.replay import (
        FORWARD_TOL,
        GRADIENT_TOL,
        _max_noise,
        _noise,
    ...
    production = _pair_pass(
        llama3_qkv_rope_forward_production, inputs, output_grads
    )
    if device.startswith("cuda"):
        torch.cuda.synchronize()
    ...
    reference = _pair_pass(llama3_qkv_rope_forward_ref, inputs, output_grads)
    ...
    if noise_repeats == 0:
        ...
        noise["measured"] = True
        passes = [
            _pair_pass(llama3_qkv_rope_forward_production, inputs, output_grads)
...
def _calibration_case(label, dtype, inputs, output_grads, multipliers, repeats: int = 3):
    ...
    names = OUTPUT_NAMES + GRAD_NAMES
    reference = _pair_pass(llama3_qkv_rope_forward_ref, inputs, output_grads)
    production = _pair_pass(llama3_qkv_rope_forward_production, inputs, output_grads)
    results = {
    ...
    for _ in range(max(repeats - 1, 0)):
        again = _pair_pass(llama3_qkv_rope_forward_production, inputs, output_grads)
        for name in names:
            noise[name] = max(
                noise[name],
                required_tolerance(
...
def run_calibration(
    ...
    *,
    ...
    return {
        "schema_version": "evograd-llama3-qkv-tolerance/1",
        "task": TASK_NAME,
        "device": device,
        "metric": (
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m evograd.benchmark.topdown.llama3_2_1b.levels.level2.qkv_rope",
        description=__doc__,
    ...
    def source_args(sp):
        sp.add_argument("--source", type=Path, default=Path("results/llama3-level4/layer8.pt"))
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        from evograd.benchmark.topdown.llama3_2_1b.levels.level3.prepare import validate_noise_repeats
        try:
