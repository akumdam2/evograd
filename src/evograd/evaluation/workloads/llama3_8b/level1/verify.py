"""Verifying the primitives Llama-3-8B runs against what the model computed.

Which primitives those are belongs to
:mod:`evograd.benchmark.topdown.llama3_8b.levels.level1.manifest`. What is here is the judgment: the SDPA
boundary captured from a live layer and checked against the declared contract,
the cross-entropy head checked against the model's own loss, a whole-step loss
sanity check, and the reports that carry each verdict.
"""
...
from evograd.benchmark.topdown.llama3_8b.levels.level1.manifest import COMPOSES_INTO, Level1Error
...
from evograd.evaluation.workloads.llama3_8b.level3.replay import compare_tensors, declared_gate, required_tolerance
...
def run_cross_entropy_check(*, device: str = "cuda", spec=None) -> dict[str, Any]:
    ...
    from evograd.evaluation.workloads.llama3_8b.level3.replay import compare_tensors
