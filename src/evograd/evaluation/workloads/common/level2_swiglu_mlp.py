"""Judging an implementation of one model's gated MLP boundary.

The case itself belongs to that model's ``levels.level2.swiglu_mlp.capture``.
...
REPORT_SCHEMA_SUFFIX = "mlp-verify/1"
...
def _bind(descriptor):
    """This model's capture module for the swiglu_mlp boundary."""
    cap = descriptor.capture(BOUNDARY)
    return cap


def _declared_case(op, *, descriptor):
    """The single observed workload for this model, or a problem describing why not."""
    cases = op.benchmark_workloads(descriptor.observed_suite)
    if len(cases) != 1:
        return None, (
            f"expected one {descriptor.observed_suite} case on {op.name}, found {len(cases)}; "
            "the declaration carries no observed suite for this workload"
        )
    return cases[0], None


def declaration_problems(snapshot_path: Path | None = None, *, descriptor) -> list[str]:
    ...
    MlpExtractionError = cap.MlpExtractionError
    check_provenance = cap.check_provenance
    derive_mlp_invocation = cap.derive_mlp_invocation
    ...
    declared, missing = _declared_case(get_task(TASK_NAME), descriptor=descriptor)
    if declared is None:
        return [missing]
    problems: list[str] = []
...
def run_verify(
    ...
    *,
    ...
    _case, _missing = _declared_case(op, descriptor=descriptor)
    if _case is None:
    ...
    shape_problems = declaration_problems(descriptor=descriptor)
    weight_shapes = {
    ...
    out, grad_x, weight_grads = _reference_pass(resolve_forward(op), payload, device)
    if device.startswith("cuda"):
        torch.cuda.synchronize()
    ...
    case, missing = _declared_case(op, descriptor=descriptor)
    if case is None:
    ...
    hf_out, hf_grad_x, hf_weight_grads = _reference_pass(
        resolve_runtime_forward(op), payload, device
    )
    hf_comparisons = {
    ...
    if noise_repeats == 0:
        ...
        noise["measured"] = True
        passes = [
            _reference_pass(resolve_forward(op), payload, device)
...
def run_calibration(
    ...
    *,
    ...
    return {
        "schema_version": descriptor.schema("mlp-tolerance/1"),
        "task": TASK_NAME,
        "device": device,
        "metric": (
...
def build_parser(descriptor) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"python -m evograd.evaluation.workloads.{descriptor.name}.level2.swiglu_mlp",
        description=__doc__,
    ...
    def source_args(sp):
        sp.add_argument(
            "--source", type=Path, default=descriptor.artifact_path
        )
...
def main(argv: list[str] | None = None, *, descriptor) -> int:
    ...
    try:
        ...
        report = run_verify(payload, device=args.device, noise_repeats=args.noise_repeats, descriptor=descriptor)
    except (MlpExtractionError, ArtifactError) as exc:
