"""Judging an implementation of one model's residual add and RMSNorm boundary.

The case itself belongs to that model's ``levels.level2.residual_rmsnorm.capture``.
...
BOUNDARY = "residual_rmsnorm"

#: Combined with each model's prefix.
REPORT_SCHEMA_SUFFIX = "residual-verify/1"
...
def _bind(descriptor):
    """This model's capture module for the residual_rmsnorm boundary."""
    cap = descriptor.capture(BOUNDARY)
    assert cap.OUTPUT_NAMES == OUTPUT_NAMES, (OUTPUT_NAMES, cap.OUTPUT_NAMES)
    assert cap.DIRECT_GRADS == DIRECT_GRADS, (DIRECT_GRADS, cap.DIRECT_GRADS)
    return cap
...
def run_verify(
    ...
    *,
    ...
    ResidualExtractionError = cap.ResidualExtractionError
    check_provenance = cap.check_provenance
    derive_residual_invocation = cap.derive_residual_invocation
    ...
    runtime = _pair_pass(resolve_runtime_forward(op), *args)
    reference = _pair_pass(resolve_forward(op), *args)
...
def _calibration_case(label, dtype, args, multipliers, repeats: int = 3):
    ...
    reference = _pair_pass(resolve_forward(op), *args)
    production = _pair_pass(resolve_runtime_forward(op), *args)
    results = {
    ...
    for _ in range(max(repeats - 1, 0)):
        again = _pair_pass(resolve_runtime_forward(op), *args)
        for name in RESULT_NAMES:
            noise[name] = max(
                noise[name],
                required_tolerance(
...
def run_calibration(
    ...
    *,
    ...
    return {
        "schema_version": descriptor.schema("residual-tolerance/1"),
        "task": TASK_NAME,
        "device": device,
        "metric": (
...
def build_parser(descriptor) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"python -m evograd.evaluation.workloads.{descriptor.name}.level2.residual_rmsnorm",
        description=__doc__,
