"""Verifying the primitives one model runs against what the model computed.

The mapping itself -- which harvested configuration becomes which generic task
-- belongs to that model's ``levels.level1.manifest``. What is here is the
judgment: does the declared primitive reproduce what the model's own step
produced, at the model's own shapes.

Written once. ``descriptor`` names the model.
"""
...
def preserve_layout_cpu(tensor):
    """A CPU copy that keeps the device tensor's exact stride.

    Byte-for-byte what each model's ``levels.level2.attention.capture`` version
    produces; the layout contract is shared, so the helper is too.
    """
    out = torch.empty_strided(
        tensor.shape, tensor.stride(), dtype=tensor.dtype, device="cpu"
    )
...
REPORT_SCHEMA_SUFFIX = "level1-verify/1"


def summarize_mapping(report: dict[str, Any], *, descriptor) -> str:
    lines = [
        f"{descriptor.label} Level-1 mapping  (snapshot {report['snapshot_hash'][:16]}...)",
...
def derive_sdpa_invocation(
    ...
    prepare_layer = descriptor.module("levels.level3.prepare").prepare_layer
    load_canonical = descriptor.module("levels.level3.artifact").load_canonical
...
def run_verify(
    ...
    *,
    ...
    capture = derive_sdpa_invocation(source, device=device, snapshot_path=snapshot_path, descriptor=descriptor)
    snapshot = descriptor.module("harvest.snapshot").load(snapshot_path)
    observed = snapshot["level1"]["causal_gqa_attention"]["configurations"][0]

    op = get_task("causal_gqa_attention")
    case = op.benchmark_workloads(descriptor.observed_suite)[0]
...
def run_loss_check(
    *, device: str = "cuda", snapshot_path: Path | None = None, descriptor
) -> dict[str, Any]:
    """Run the observed cross entropy at its real shape without persisting it.

    ``[4096, 128256]`` float32 logits are 2.1 GiB; a stored artifact of them
    ...
    freshly initialised model's loss sits near ``ln(vocab)``, which for this model's
    128256-token vocabulary is 11.76. The measured value comes from the run
    rather than from here -- an expected constant written down in advance is not
    a check.
    ...
    report = {
        "schema_version": descriptor.schema("level1-loss/1"),
        "task": "cross_entropy",
        "dims": case.dims,
        "dtype": case.dtype,
        ...
        # Llama-3-8B's own canonical loss, recorded by the harvest on manifest
        # c235f1bad3fec0cf under transformers 5.16.1. Qwen3's is
        # 12.14388656616211 and was what this line carried; the two workloads
        # have different vocabularies, so neither is a reference for the other.
        # This is a printed reference value, not a gate -- and it is
        # environment-sensitive: the same step under transformers 4.57.6 took a
        # different SDPA path and produced 12.594093322753906.
        "canonical_level4_loss": descriptor.canonical_loss,
...
def run_cross_entropy_check(*, device: str = "cuda", spec=None, descriptor) -> dict[str, Any]:
    ...
    Nothing is persisted. The ``[4096, 128256]`` logits and their gradient are
    2.1 GiB each; they are compared inside the hooks that see them and dropped
    ...
    _level4 = descriptor.module("levels.level4.model")
    build_model, make_inputs = _level4.build_model, _level4.make_inputs
    require_transformers, training_step = _level4.require_transformers, _level4.training_step
    from evograd.evaluation.workloads.common.replay import compare_tensors
    CANONICAL = descriptor.module("levels.level4.spec").CANONICAL
    ...
    return {
        "schema_version": descriptor.schema("level1-cross-entropy/1"),
        "status": "pass" if not failures else "fail",
        "failures": failures,
        "task": "cross_entropy",
