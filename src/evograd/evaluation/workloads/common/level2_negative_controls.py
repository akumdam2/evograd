"""Does a calibrated gate still reject a wrong kernel?

A tolerance that accepts everything is not a tolerance. Widening one to admit
BF16's real behaviour is only defensible if the widened gate still catches the
errors implementations actually make, so every tolerance family that moved gets
an injected fault and has to fail.

Written once; ``descriptor`` names the model. The ``unreached`` accounting below
came from the Llama work and now applies to every model: a fault that cannot
reach a result is reported as inapplicable rather than as a detection, because
scoring it as a hit credits the gate with sensitivity it never had.
"""
...
def dropped_term_fault(op, workload, clean, values, reference) -> dict[str, Any]:
    ...
    A result the fault cannot reach is reported as inapplicable rather than as
    undetected. the q/k/v boundary's ``dx`` is the case that forced this: with
    no per-head norm, ``dx = dq_flat @ q_weight + dk_flat @ k_weight +
    dv_flat @ v_weight`` reads no ``x`` at all, so zeroing a token of ``x``
    leaves it bit-identical and no tolerance could ever "catch" it. Scoring that
    as a miss would put a permanent red mark next to a gate that is fine, and --
    worse -- scoring it as a hit, which is what happens when the clean pair is
    already rejected, would credit the gate with a detection it never made.
    """
    from evograd.evaluation.workloads.common.level2_calibrate import production_results
    ...
    for result_name in (*op.output_names, *op.grad_names()):
        if torch.equal(faulty[result_name], clean[result_name]):
            # The fault never reached this result. Not a detection failure.
            caught[result_name] = None
            unreached.append(result_name)
            continue
...
def run_task(task: str, *, device: str = "cuda", descriptor) -> dict[str, Any]:
    from evograd.evaluation.workloads.common.level2_calibrate import (
        production_results,
        reference_results,
    )
    ...
    op = get_task(task)
    workloads = list(op.benchmark_workloads(suite=descriptor.observed_suite))
    ...
    for workload in workloads:
        ...
        dropped = dropped_term_fault(op, workload, clean, values, reference)
        out.append(
            {
...
def main(argv: list[str] | None = None, *, descriptor) -> int:
    parser = argparse.ArgumentParser(
        prog=f"python -m evograd.evaluation.workloads.{descriptor.name}.level2.negative_controls",
        description=__doc__,
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--task", action="append", default=None, choices=descriptor.level2_tasks)
    parser.add_argument("--device", default="cuda")
    parser.add_argument("--report", type=Path, default=None)
    args = parser.parse_args(sys.argv[1:] if argv is None else argv)

    report = {
        "schema_version": descriptor.schema("l2-negative-control/1"),
        "device": args.device,
        "scale_faults_tried": list(SCALE_FAULTS),
        "tasks": [
            run_task(t, device=args.device, descriptor=descriptor)
            for t in (args.task or descriptor.level2_tasks)
        ],
    }
