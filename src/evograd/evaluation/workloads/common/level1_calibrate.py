"""Measuring the tolerance one model's Level-1 primitive actually needs.

Written once; ``descriptor`` names the model. See
:mod:`evograd.evaluation.workloads.common.descriptor`.
"""
...
from evograd.evaluation.workloads.common.replay import required_tolerance
...
def _level1_error(descriptor):
    """This model's Level-1 error type, from its benchmark package."""
    return descriptor.module("levels.level1.manifest").Level1Error


def run_calibration(
    op_name: str, *, device: str = "cuda", repeats: int = 3, descriptor
) -> dict[str, Any]:
    ...
    if not op.runtime_forward:
        raise _level1_error(descriptor)(f"{op_name} has no runtime_forward to calibrate against")
    reference_fn = resolve_forward(op)
    production_fn = resolve_runtime_forward(op)
    names = _result_names(op)

    workloads = list(op.correctness) + list(op.benchmark_workloads(descriptor.observed_suite))
    cases = []
        ...
        cases.append(
            {
                ...
                and workload.provenance.model == descriptor.name,
                "results": results,
    ...
    return {
        "schema_version": descriptor.schema("level1-tolerance/1"),
        "task": op_name,
        "device": device,
        "metric": (
