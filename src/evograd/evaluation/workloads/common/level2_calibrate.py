"""Numerical calibration for one model's observed Level-2 boundaries.

One question, asked of each declared boundary: **what does a correct
implementation actually have to be allowed, at the shape the model runs?**
...
*identical* upstream gradients. A tolerance derived here is therefore the floor:
no implementation of these semantics can agree with the oracle more closely than
the reference implementation does, so anything tighter would reject PyTorch.

Three populations, because they answer different halves of the question: the
declared correctness workloads, the model's observed workloads, and the
harvested invocation whose tensors are the model's own rather than synthetic --
which is how a scale problem is told apart from a spelling problem.

Written once. ``descriptor`` is the model; note that ``workload`` throughout
means an :class:`~evograd.opdecl.activity.Workload` -- one benchmark shape case
-- which is why the model is never called that here.
"""
...
def _observed_workloads(op, *, descriptor):
    try:
        return op.benchmark_workloads(suite=descriptor.observed_suite)
    except Exception:
        return ()
...
def inventory_task(
    ...
    *,
    ...
    for workload in _observed_workloads(op, descriptor=descriptor):
        values = make_case_inputs(op, workload, device=device)
        cases.append(
    ...
    if include_harvested and artifact is not None and artifact.is_file():
        harvested = _harvested_case(op, artifact, device=device, repeats=repeats, descriptor=descriptor)
    ...
    return {
        ...
        "provenance": {
            ...
            "forward": op.forward,
            "snapshot_hash": _snapshot_hash(descriptor=descriptor),
        },
        "cases": cases,
...
def _snapshot_hash(*, descriptor) -> str | None:
    try:
        load = descriptor.module("harvest.snapshot").load
...
def _harvested_case(op, artifact: Path, *, device: str, repeats: int, descriptor):
    ...
    builders = {
        ...
        descriptor.level2_tasks[0]: _harvested_qkv,
    }
    build = builders.get(op.name)
    if build is None:
        return None
    try:
        values, workload = build(op, artifact, device=device, descriptor=descriptor)
    except Exception as exc:  # a missing or stale capture is not a failure here
        return {
            "label": f"harvested layer-{descriptor.representative_layer} (unavailable)",
            "error": f"{type(exc).__name__}: {exc}",
            "ok": True,
            "results": {},
            "worst_required_scale": 0.0,
        }
    record = case(op, f"harvested layer-{descriptor.representative_layer} invocation", workload, values, repeats=repeats)
    record["source"] = str(artifact)
    return record


def _observed_workload_for(op, *, descriptor):
    observed = _observed_workloads(op, descriptor=descriptor)
    return observed[0] if observed else op.benchmark[0]


def _harvested_mlp(op, artifact: Path, *, device: str, descriptor):
    derive_mlp_invocation = descriptor.capture("swiglu_mlp").derive_mlp_invocation
    ...
    return values, _observed_workload_for(op, descriptor=descriptor)


def _harvested_attention(op, artifact: Path, *, device: str, descriptor):
    derive_attention_invocation = descriptor.capture("attention").derive_attention_invocation
...
def _harvested_qkv(op, artifact: Path, *, device: str, descriptor):
    derive_qkv_invocation = descriptor.capture(descriptor.qkv_module).derive_qkv_invocation
...
def scaling_study(
    task: str, *, device: str = "cuda", repeats: int = 2, descriptor
) -> dict[str, Any]:
    ...
    op = get_task(task)
    base = _observed_workload_for(op, descriptor=descriptor)
    token_dim = "T"
...
def build_parser(descriptor) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=f"python -m evograd.evaluation.workloads.{descriptor.name}.level2.calibrate",
        description=__doc__,
    ...
    for name in ("inventory", "scaling"):
        ...
        sp.add_argument("--task", action="append", default=None, choices=descriptor.level2_tasks)
        ...
        sp.add_argument(
            "--artifact",
            type=Path,
            default=descriptor.artifact_path,
...
def main(argv: list[str] | None = None, *, descriptor) -> int:
    args = build_parser(descriptor).parse_args(sys.argv[1:] if argv is None else argv)
    tasks = tuple(args.task) if args.task else descriptor.level2_tasks

    report: dict[str, Any] = {
        "schema_version": descriptor.schema("l2-calibration/1"),
    ...
    if args.command == "inventory":
        report["tasks"] = [
            inventory_task(
                ...
                artifact=args.artifact, descriptor=descriptor,
            )
            for task in tasks
        ]
    else:
        report["tasks"] = []
        report["scaling"] = [
            scaling_study(task, device=args.device, repeats=max(args.repeats // 2, 2), descriptor=descriptor)
            for task in tasks
        ]
